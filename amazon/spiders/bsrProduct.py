...
class BaseInfo(Spider):
    ...
    # start_urls =     [
    #
    #     'https://www.amazon.com/dp/B00A9JHQ1I'
    #
    #
    # ]
    ...
    custom_settings = {
        'ITEM_PIPELINES': {
            ...
            'amazon.pipelines.KafkaPipelineBSRSimulateSeed': 301,
        }
    }
    ...
    def parse(self, response):
        item = {'asin': '', 'url': "", 'parentasin': '', 'scrapyTime': '', 'marketplace': '', 'reviewsnum': '-1',
                'stars': '-1', 'star5': '-1%',
                'star4': '-1%', 'star3': '-1%', 'star2': '-1%', 'star1': '-1%', 'questionsnum': '-1', 'imgurl': [], 'brand': '',
                'flag': '', 'asinname': '',
                'price': '-1', 'cur': '', 'bulletpoints': '', 'variationname': '', 'publishedtime': '', 'sellername': '',
                'sellerid': '',
                'shippingmethod': 'Fulfilled by Manufacturers', 'category': '','bsr':'-1','subcategory': [], 'childBsr': [], 'sellerCount': '-1',
                'shipAdress': '',
                'spec': {'dimensions': '', 'weight': ''}}

        try:
            # asin
            asinList = ["//span[@class='a-declarative']/@data-a-modal","//div[@id='tell-a-friend']//span[@class='a-declarative']/@data-a-modal"]
            ...
            if item['asin'] is '' or item['asin'] is None:
                ...
                if len(sellerATag) > 0:
                    ...
                    if name and name.strip() != "":
                        if 'sold by' in name:
                            ...
                            item['sellername'] = sellername.replace('\n', '')
                            item['sellerid'] = 'Amazon'
                            item['shippingmethod'] = 'Fulfilled by Amazon'
                # 获取子分类的第一种方式
                subCategorysItem = []
                ...
                if len(subCategorys) > 0:
                    ...
                    if " in " in mainSubCategory:
                        ...
                        cgDict['name'] = mainSubCategoryList[1].strip()
                        cgList.append(cgDict)
                    _subCategorys = response.xpath('//*[@id="SalesRank"]/td/ul[@class="zg_hrsr"]/li[@class="zg_hrsr_item"]')
                    _subCategorysList = _subCategorys.xpath('string(.)').extract()
                    for sg in _subCategorysList:
                        sgs = sg.replace("\xa0", " ").split(" in ")
                        cgDict = {}  # {name='分类1'，bsr='排名1'}
